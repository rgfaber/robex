defmodule FileSystem.Mixfile do
  ...
  def project do
    [
      ...
      aliases: [
        "compile.file_system": &file_system/1
      ],
      docs: [
        extras: ["README.md"],
        main: "readme",
  ...
  defp compile_mac do
    ...
    if Mix.Utils.stale?(Path.wildcard(source), [target]) do
      ...
      cmd = "clang -framework CoreFoundation -framework CoreServices -Wno-deprecated-declarations #{source} -o #{
        target
      }"
      if Mix.shell().cmd(cmd) > 0 do
        Logger.error "Could not compile file system watcher for Mac, try to run #{
          inspect cmd
  ...
  defp package do
    %{
      ...
      licenses: ["WTFPL"],
      links: %{
        "Github" => "https://github.com/falood/file_system"
      }
    }
  end
end
